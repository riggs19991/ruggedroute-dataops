...
export function Groups() {
  ...
  return (
    <>
      <h1>Groups</h1>
      <p className="muted">A group is one class. The teacher posts each week's material and shares files; students join with a code.</p>
      {msg && <div className={`notice ${msg.kind}`}>{msg.text}</div>}

      <div className="two-col">
        <form className="form" onSubmit={join}>
          <h3>Join a group</h3>
          <div className="field"><label>Join code from your teacher</label><input type="text" value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="e.g. K7M2PX" required style={{ fontFamily: 'var(--mono)', letterSpacing: '0.1em' }} /></div>
          <button type="submit" className="btn primary">Join</button>
        </form>
        {isTeacher && (
          <form className="form" onSubmit={create}>
            <h3>Create a group (teacher)</h3>
      ...
      <section className="section">
        ...
        {groups === null ? <p className="loading">Loading…</p> : groups.length === 0 ? (
          <div className="empty">You are not in a group yet. {isTeacher ? 'Create one above.' : 'Ask your teacher for the join code.'}</div>
        ) : (
          <div className="grid">
            {groups.map((g) => {
