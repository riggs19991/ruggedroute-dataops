...
export function SupportAsk({ compact = false }: { compact?: boolean }) {
  const button = DONATE_URL
    ? <a className="btn primary small donate" href={DONATE_URL} target="_blank" rel="noopener noreferrer">☕ Support the creator</a>
    : <Link className="btn primary small donate" to={donateHref}>☕ Support the creator</Link>
